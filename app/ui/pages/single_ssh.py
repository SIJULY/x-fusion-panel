...
async def render_single_ssh_view(server_conf):
            ...
            with ui.column().classes('w-full p-5 gap-4').style('background: var(--xf-bg-main);'):
                ...
                with ui.element('div').classes(
                    ...
                    cmd_input = ui.textarea('执行命令', value=existing_cmd['cmd'] if existing_cmd else '').classes(
                        'w-full').props(
                        'dense outlined dark color=cyan standout bg-color="[#050b14]" rows=4' if is_dark else 'dense outlined color=blue rows=4')
    ...
    with content_container:
        # 修改点 1：外层容器增加 gap-4 min-h-0
        with ui.column().classes('w-full max-w-[1440px] mx-auto h-full flex flex-col gap-4 flex-nowrap min-h-0'):

            # 修改点 2：SSH 卡片改为 flex-1 弹性伸缩
            with ui.card().classes(
                    'w-full p-0 rounded-sm border border-t-[3px] overflow-hidden flex flex-col flex-1 min-h-[200px]').style(
                ...
                with ui.row().classes(
                        'w-full items-center justify-between px-4 py-1 border-b min-h-[32px]').style(
                    'background: var(--xf-code-bg); border-color: var(--xf-card-border);'):
                    with ui.row().classes('items-center gap-2'):
                        ui.badge('独立路由终端', color='green').props('outline rounded-sm').classes(
                            'text-[10px] font-black tracking-wide text-green-400')
                        ui.badge('交互模式', color='blue').props('outline rounded-sm').classes(
                            'text-[10px] font-black tracking-wide text-cyan-300')
                    render_conn_btn()

                # 修改点 3：移除 SSH 黑框的固定高度，改为 flex-1 撑满
                terminal_box = ui.element('div').classes('w-full flex-1 min-h-0 relative border-t').style(
                    'background: var(--xf-code-bg); border-top-color: var(--xf-card-border);')
                with terminal_box:
                    with ui.column().classes('w-full h-full items-center justify-center text-slate-500'):
                        ui.label('正在初始化 SSH 终端...').classes('text-sm')

            # 修改点 4：快捷命令栏保持自身高度（shrink-0），去除 mt-4
            with ui.card().classes(
                    'w-full p-4 rounded-sm border overflow-hidden flex flex-col shrink-0').style(
                'background: var(--xf-panel-bg); border-color: var(--xf-card-border); box-shadow: 0 10px 28px rgba(15,23,42,0.18);'):
                render_quick_commands()

            # 修改点 5：文件管理卡片改为 flex-1，并去除固定高度与 mt-4
            with ui.card().classes(
                    'w-full p-0 rounded-sm border overflow-hidden flex flex-col flex-1 min-h-[250px]').style(
                'background: var(--xf-panel-bg); border-color: var(--xf-card-border); box-shadow: 0 10px 28px rgba(15,23,42,0.18);'):
                with ui.row().classes(
                ...
                # 修改点 6：文件管理内部改为 flex-1 弹性填充
                with ui.row().classes('w-full flex-1 min-h-0 flex-nowrap no-wrap gap-0'):
