...
X-Fusion Panel 是一个面向 **多服务器运维 / VPS 管理 / 节点管理 / 订阅分发 / 探针监控** 场景的可视化管理面板，基于 **NiceGUI + FastAPI + Paramiko** 构建。
...
| 面板 API 接入 | ✅ | 支持对接现有面板 API 进行节点获取、编辑、删除 |
...
- **兼顾 API 管理与 Root 运维**：不仅能对接现有面板 API，也能走 SSH/Root 模式处理更底层的场景
...
如果你只想通过 API 做基础节点管理，也可以不装探针；但探针装上后，状态、延迟、自动同步体验会更完整。
